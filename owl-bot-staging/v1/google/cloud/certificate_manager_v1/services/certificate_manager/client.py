...
class CertificateManagerClient(metaclass=CertificateManagerClientMeta):
    ...
    @staticmethod
    def target_https_proxies_path(project: str,location: str,targetHttpsProxy: str,) -> str:
        """Returns a fully-qualified target_https_proxies string."""
        return "projects/{project}/locations/{location}/targetHttpsProxies/{targetHttpsProxy}".format(project=project, location=location, targetHttpsProxy=targetHttpsProxy, )

    @staticmethod
    def parse_target_https_proxies_path(path: str) -> Dict[str,str]:
        """Parses a target_https_proxies path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/targetHttpsProxies/(?P<targetHttpsProxy>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def target_ssl_proxies_path(project: str,location: str,targetSslProxy: str,) -> str:
        """Returns a fully-qualified target_ssl_proxies string."""
        return "projects/{project}/locations/{location}/targetSslProxies/{targetSslProxy}".format(project=project, location=location, targetSslProxy=targetSslProxy, )

    @staticmethod
    def parse_target_ssl_proxies_path(path: str) -> Dict[str,str]:
        """Parses a target_ssl_proxies path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/targetSslProxies/(?P<targetSslProxy>.+?)$", path)
        return m.groupdict() if m else {}
